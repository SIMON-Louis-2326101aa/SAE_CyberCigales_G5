...
class EmailVerificationModelIntegrationTest extends DatabaseTestCase
{
    ...
    protected function setUp(): void // Méthode appelée AVANT chaque test
    {
        ...
        $this->model = new EmailVerificationModel(); // Crée nouvelle instance EmailVerificationModel pour chaque test
    }

    /**
     * @testdox Génère et stocke un code de vérification valide de 6 chiffres en base de données (generateAndStoreCode()
     * crée un code, l'insère dans email_verification_codes avec expires_at, puis vérifie en base avec SELECT)
     */
    public function testGenerateAndStoreCodeCreatesValidCode(): void // Test : generateAndStoreCode()
    {
        $email = 'test@example.com'; // Email de test pour lequel générer le code

        $code = $this->model->generateAndStoreCode($email, 10); // Génère un code de 6 chiffres,
        // le stocke en base avec expiration (10 min), retourne le code
        ...
        $this->assertMatchesRegularExpression('/^\d{6}$/', $code); // Vérifie que $code contient exactement 6 chiffres
        // (0-9) avec une regex

        // Prépare la requête pour chercher le code en base
        $stmt = $this->pdo->prepare('SELECT * FROM email_verification_codes WHERE email = ? AND code = ?');

        $stmt->execute([$email, $code]); // Exécute la requête avec l'email et le code

        $result = $stmt->fetch(\PDO::FETCH_ASSOC); // Récupère la ligne sous forme de tableau associatif,
        // stocke dans $result
    ...
    /**
     * @testdox Retourne un statut valide pour un code de vérification correct (génère un code, puis checkCodeStatus()
     * le compare avec celui en base, retourne ['valid' => true, 'reason' => 'valid'])
     */
    public function testCheckCodeStatusReturnsValidForCorrectCode(): void // Test : checkCodeStatus()
    {
        ...
        $status = $this->model->checkCodeStatus($email, $code); // Vérifie statut code, stocke le résultat dans $status
    ...
    /**
     * @testdox Retourne un statut invalide pour un code de vérification incorrect (génère un code,
     * puis checkCodeStatus() avec un code différent retourne ['valid' => false, 'reason' => 'incorrect'])
     */
    public function testCheckCodeStatusReturnsIncorrectForWrongCode(): void // Test : checkCodeStatus()
    {
        $email = 'test@example.com'; // Email de test

        $this->model->generateAndStoreCode($email, 10); // Génère et stocke un code pour cet email

        $status = $this->model->checkCodeStatus($email, '999999'); // Vérifie le statut avec un MAUVAIS code,
        // stocke dans $status
    ...
    /**
     * @testdox Crée un enregistrement dans pending_registrations pour une inscription en attente
     * (storePendingRegistration() insère nom, prenom, email, password hashé, puis vérifie avec
     * SELECT que les données sont correctes)
     */
    public function testStorePendingRegistrationCreatesRecord(): void // Test : storePendingRegistration()
    {
        ...
        // Stocke l'inscription en attente, stocke le résultat dans $result
    ...
    /**
     * @testdox Un nouveau code de vérification remplace l'ancien pour le même email
     * (génère un premier code, compte les codes, génère un second code,
     * les deux codes sont différents mais le nouveau existe en base)
     */
    public function testGeneratingNewCodeReplacesOldOne(): void // Test : générer nouveau code même email crée code diff
    {
        ...
        // Prépare la requête pour compter les codes
        ...
        // Vérifie que le nombre de codes est >= au nombre initial (peut y avoir plusieurs codes)
        $this->assertGreaterThanOrEqual($count1, $count2);
    }

    /**
     * @testdox Limite le TTL entre 1 et 60 minutes (valeurs invalides sont ajustées)
     * (génère des codes avec TTL 0, 100 et 15, le système limite automatiquement à 1-60,
    ...
    public function testTtlIsClampedToValidRange(): void // Test : le TTL est limité entre 1 et 60 minutes
    {
        ...
        // Génère un code avec TTL trop petit (0) - devrait être remonté à 1
        $this->model->generateAndStoreCode($email, 0);

        // Génère un code avec TTL trop grand (100) - devrait être limité à 60
    ...
    /**
     * @testdox Récupère une inscription en attente (getPendingRegistration() fait un
     * SELECT * FROM pending_registrations WHERE email = ?, retourne un tableau ou null)
     */
