<main>
    ...
    <?php if (!empty($devCode)) : ?>
        <div style="background-color: #fff3e0; border: 2px solid #ff9800; padding: 15px;
        border-radius: 5px; margin: 15px 0;">
            <p style="color: #e65100; font-weight: bold; margin: 0;">🔑 <strong>Code (dev/local):</strong>
                <?php echo htmlspecialchars($devCode); ?></p>
        </div>
    <?php endif; ?>

    <?php if (!empty($error)) : ?>
        <div style="background-color: #ffebee; border: 1px solid #f44336; padding: 15px;
        border-radius: 5px; margin: 15px 0;">
    ...
    <p>
        <a href="index.php?controller=EmailVerification&action=request&email=
        <?php echo urlencode($email ?? ''); ?>">Renvoyer un code</a>
    </p>
</main>
</body>
</html>
